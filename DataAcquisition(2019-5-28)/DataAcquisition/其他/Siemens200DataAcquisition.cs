...
{
    class Siemens200DataAcquisition
    {
        //读取西门子S7200系列PLC数据（Modbus Tcp协议）
        public Dictionary<string, float> AcquireData(string ip, int port)
        {
            ...
            //解析第一次回应
            byte[] result = new byte[4] { FirstResponse[12], FirstResponse[11], FirstResponse[10], FirstResponse[9] };
            //Array.Reverse(result);
            float Jumping = BitConverter.ToSingle(result, 0);
